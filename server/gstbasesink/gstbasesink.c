    ...
    GstClockTimeDiff * jitter)
{
  ...
  do {
    ...
    GST_ERROR_OBJECT (sink, "clock returned %d", status);
...
//doing nothing
  ...
  if (priv->rc_next == -1 || (stime != -1 && stime >= priv->rc_next)) {
	//in here
    GST_ERROR_OBJECT (basesink, "reset rc_time to time %" GST_TIME_FORMAT,
        GST_TIME_ARGS (stime));
    priv->rc_time = stime;
  ...
  /* preroll done, we can sync since we are in PLAYING now. */
  GST_ERROR_OBJECT (basesink, "possibly waiting for clock to reach %"
      GST_TIME_FORMAT ", adjusted %" GST_TIME_FORMAT,
      GST_TIME_ARGS (rstart), GST_TIME_ARGS (stime));
//no use
//stime=stime - gst_util_uint64_scale_round ( 1,GST_SECOND,30);
...
/*--------------------------@@@@ 25 ms - 30 ms at here ---------------------------*/
  ...
  GST_ERROR_OBJECT (basesink, "clock returned %d, jitter %c%" GST_TIME_FORMAT,

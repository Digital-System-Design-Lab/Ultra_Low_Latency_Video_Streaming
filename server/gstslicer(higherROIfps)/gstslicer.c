...
int routine=4; //1,2,3-> roi only 4-> all

//syd_ split video into slices and push it to next plugin
//ROI slice and Non ROI slice have different fps
//push all slices only once in 4 times and push only ROI slices in 3 times in 4 times
...
gst_slicer_process (GstSlicer * self, GstBuffer * buf)
{
  ...
  if(routine%4==0){
	...
	for(int i=0;i<GST_TILE_NUMBER;i++){
		buffers_out[i]=gst_buffer_copy_region(buf,GST_BUFFER_COPY_MEMORY, i * bufsize,bufsize );
 		gst_buffer_copy_into (buffers_out[i], buf, GST_BUFFER_COPY_METADATA, 0,-1);
		gst_buffer_copy_into (buffers_out[i], buf, GST_BUFFER_COPY_TIMESTAMPS, 0,-1);
	}
  ...
  //push only ROI slices (2,3)
  else{

  	routine++;

	/** slicing only ROI start **/
  	for(int i=0;i<GST_TILE_NUMBER;i++){
	...
	/** end slicing only ROI **/
  	...
  	for(srcs = self->srcpads, i = 0; srcs; srcs = srcs->next, i++){
		if(i==2){
			temp[0]._pad=srcs->data;
			temp[0]._buf=buffers_out[i];
			pthread_create(&threads[0],NULL,process,(void *)&temp[0]);
		}
	
		if(i==3){
			temp[1]._pad=srcs->data;
			temp[1]._buf=buffers_out[i];
			pthread_create(&threads[1],NULL,process,(void *)&temp[1]);
		}
  	}
